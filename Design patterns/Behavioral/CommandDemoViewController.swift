//
//  CommandDemoViewController.swift
//  Design patterns
//
...
class CommandDemoViewController: UIViewController {
    ...
    private func setupUI() {
        ...
        RemoteControl（调用者，存储和执行命令）
        Light（接收者，执行具体操作）
        
        ⚙️ 执行流程：调用者发送命令，命令对象执行并支持撤销
        """
        ...
        let onButton = createButton(title: "开灯", action: #selector(turnOn))
        let offButton = createButton(title: "关灯", action: #selector(turnOff))
        let undoButton = createButton(title: "撤销", action: #selector(undo))
        
        buttonStackView.addArrangedSubview(onButton)
        buttonStackView.addArrangedSubview(offButton)
    ...
    private func demonstratePattern() {
        ...
        appendOutput("  命令模式演示")
        appendOutput("═══════════════════════════════════════")
        appendOutput("📝 命令模式将请求封装成对象")
        appendOutput("💡 支持撤销操作和参数化请求")
        appendOutput("请点击上方按钮测试命令执行和撤销")
    }
    
    @objc private func turnOn() {
        appendOutput("\n--- 执行开灯命令 ---")
        appendOutput("  步骤1: 创建开灯命令对象")
        let command = LightOnCommand(light: light)
        appendOutput("  步骤2: 设置远程控制器命令")
        remoteControl.setCommand(command: command)
        appendOutput("  步骤3: 执行命令")
        if let result = remoteControl.pressButton() {
            appendOutput("  \(result)")
        }
        appendOutput("✅ 开灯命令执行完成")
    }
    
    @objc private func turnOff() {
        appendOutput("\n--- 执行关灯命令 ---")
        appendOutput("  步骤1: 创建关灯命令对象")
        let command = LightOffCommand(light: light)
        appendOutput("  步骤2: 设置远程控制器命令")
        remoteControl.setCommand(command: command)
        appendOutput("  步骤3: 执行命令")
        if let result = remoteControl.pressButton() {
        ...
        appendOutput("✅ 关灯命令执行完成")
    }
    
    @objc private func undo() {
        appendOutput("\n--- 撤销操作 ---")
        appendOutput("  通过命令对象执行撤销")
        if let result = remoteControl.pressUndo() {
            appendOutput("  \(result)")
        }
        appendOutput("✅ 撤销操作完成")
    }
...
extension CommandDemoViewController: UIScrollViewDelegate {
