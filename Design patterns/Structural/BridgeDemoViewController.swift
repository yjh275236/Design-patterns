...
class BridgeDemoViewController: UIViewController {
    ...
    private func demonstratePattern() {
        ...
        appendOutput("  桥接模式演示")
        appendOutput("═══════════════════════════════════════")
        appendOutput("📝 桥接模式分离抽象和实现")
        appendOutput("💡 形状和颜色可以独立变化")
        appendOutput("请点击上方按钮测试不同的形状和颜色组合")
    }
    
    @objc private func drawRedCircle() {
        appendOutput("\n--- 绘制红色圆形 ---")
        appendOutput("  步骤1: 创建红色颜色对象")
        let color = Red()
        appendOutput("  步骤2: 创建圆形形状，桥接颜色")
        let circle = BridgeCircle(color: color)
        appendOutput("  步骤3: 绘制图形")
    ...
    @objc private func drawBlueSquare() {
        appendOutput("\n--- 绘制蓝色正方形 ---")
        appendOutput("  步骤1: 创建蓝色颜色对象")
        let color = Blue()
        appendOutput("  步骤2: 创建正方形形状，桥接颜色")
