...
class AdapterDemoViewController: UIViewController {
    ...
    private func setupUI() {
        ...
        ├── AudioPlayer（具体播放器）
        └── MediaAdapter（适配器）
            └── AdvancedMediaPlayer（高级播放器，需要适配）
        
        ⚙️ 执行流程：客户端通过适配器使用不兼容的接口，适配器进行转换
        """
...
extension AdapterDemoViewController: UIScrollViewDelegate {
